...
func initWorkspace(options initOptions, out io.Writer) error {
...
func validateWorkspace(out io.Writer) error {
...
func listWorkspace(path string, includeArchived bool, out io.Writer) error {
	ws, err := loadWorkspace(false)
	if err != nil {
		return err
	}
	query := NodeQuery{Path: path, IncludeArchived: includeArchived}
...
func infoWorkspace(path string, includeArchived bool, out io.Writer) error {
	ws, err := loadWorkspace(false)
	if err != nil {
		return err
	}
...
func dependenciesWorkspace(path string, includeOptional bool, includeArchived bool, out io.Writer) error {
	ws, err := loadWorkspace(false)
	if err != nil {
		return err
	}
...
func cloneWorkspace(path string, includeOptional bool, includeArchived bool, out io.Writer) error {
	ws, err := loadWorkspace(true)
	if err != nil {
		return err
	}
	if err := clonePathIntoWorkspace(out, ws, path, includeOptional, includeArchived); err != nil {
		return err
	}
...
func syncCurrentWorkspace(path string, includeOptional bool, includeArchived bool, out io.Writer) error {
	ws, err := loadWorkspace(true)
	if err != nil {
		return err
	}
	return syncWorkspace(out, ws, path, includeOptional, includeArchived)
}
...
func pullWorkspace(path string, includeArchived bool, out io.Writer) error {
	ws, err := loadWorkspace(false)
	if err != nil {
		return err
	}
...
func statusWorkspace(path string, includeArchived bool, out io.Writer) error {
	ws, err := loadWorkspace(false)
	if err != nil {
		return err
	}
	...
	activeFiles := activeFilesForRepoSet(&ws.Model, map[string]bool{}, installedNodes.Repos, installedNodes.Files, includeArchived)
...
func updateWorkspace(options updateOptions, out io.Writer) error {

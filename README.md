...
- **`$dir`** — a whole subtree materialized from a source repo. `src` may also be a list of sources merged in order (first wins on conflicts) — e.g. one `.agents/skills` assembled from several skill repositories. Jig tracks a manifest of what it wrote, so updates touch only unmodified files and user files inside are never touched.
